...
AMOUNT_PATTERNS = [
    ...
    re.compile(r"(?:grand\s+total|total\s+amount|amount\s+paid|net\s+total)\s*\$?\s*(-?\d[\d,]*\.\d{2})", re.IGNORECASE),
...
def detect_pdftotext() -> Optional[Path]:
    ...
    repo_candidates = [
        repo_root / "tools" / "poppler" / "Library" / "bin" / "pdftotext.exe",
        repo_root / "tools" / "poppler" / "bin" / "pdftotext.exe",
    ]
    for cand in repo_candidates:
        if cand.exists():
            candidates.append(cand)
    poppler_root = repo_root / "tools" / "poppler"
...
def extract_portal_url(text: str) -> Optional[str]:
    ...
    try:
        ...
        if raw:
            return clean_portal_url(unquote(raw[0]))
    except Exception:
        return None
    return None
